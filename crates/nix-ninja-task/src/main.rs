...
fn main() -> Result<()> {
    ...
    // Directories the driver asks for, created AFTER create_symlinks so a
    // staged input's own parent is never raced. Two populations: a generator
    // may write a side product into a build-tree directory that exists empty
    // (rdma-core's pandoc-prebuilt cache) and nothing else here would create
    ...
    if let Ok(raw) = env::var("NIX_NINJA_MAKE_DIRS") {
        ...
        // unreachable whatever the driver carried, and glib's gio compiles
