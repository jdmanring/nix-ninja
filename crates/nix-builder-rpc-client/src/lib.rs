...
impl BuilderRpcClient {
    ...
    /// one, so this is a concurrency bound in its own right and must not be
    /// left to sit below `-j`: the default is `available_parallelism() + 1`,
    /// a number nobody chose, and every `-j` above it silently became that
    /// instead.
    ...
    /// Takes the store directory because printing a derivation's aterm form
    /// needs it; the one-argument shape could not reach it.
