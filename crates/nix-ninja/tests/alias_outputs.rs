...
/// THE ALIAS ARRIVED AND ITS TARGET DID NOT. openfec's `all` names the
/// unversioned alias phony, so placement received `libopenfec.so` and
/// `libopenfec.so.1` as link text and never the library they name, which no
/// target requested; the installer copied through a dangling link
/// (configuration B, 2026-09-04). The placement step asks each placed link
/// for its target and pulls that output in, one link per round.
#[test]
fn a_placed_link_pulls_in_the_output_it_names() {
    use harmonia_store_path::StoreDir;
    use nix_ninja::local::link_targets_to_place;
    use nix_ninja_task::derived_file::DerivedFile;
    use std::collections::{HashMap, HashSet};

    let d = dir("linktarget");
    let store_root = d.join("store");
    let store_dir = StoreDir::new(&store_root).unwrap();
    let enc = |h: &str, name: &str, rel: &str| {
        let sp = format!("{}/{h}-{name}", store_root.display());
    ...
    let h_alias = "1aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    let h_lib = "1bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
    let alias_obj = store_root.join(format!("{h_alias}-ninja-build-libopenfec.so.1"));
    std::fs::create_dir_all(alias_obj.join("bin/Release")).unwrap();
    std::os::unix::fs::symlink(
        "libopenfec.so.1.4.2",
        alias_obj.join("bin/Release/libopenfec.so.1"),
    )
    .unwrap();
    let alias = enc(
        h_alias,
        "ninja-build-libopenfec.so.1",
        "bin/Release/libopenfec.so.1",
    );
    let lib = enc(
        h_lib,
        "ninja-build-libopenfec.so.1.4.2",
        "bin/Release/libopenfec.so.1.4.2",
    );

    let all: HashMap<_, _> = [(lib.build_path.clone(), lib.clone())]
        .into_iter()
        .collect();
    let have: HashSet<_> = [alias.build_path.clone()].into_iter().collect();
    let extra = link_targets_to_place(std::slice::from_ref(&alias), &store_dir, &all, &have);
    assert_eq!(extra.len(), 1, "the library the alias names is pulled in");
    assert_eq!(extra[0].build_path, lib.build_path);

    // Already in the placement set: nothing to add.
    let have_both: HashSet<_> = [alias.build_path.clone(), lib.build_path.clone()]
        .into_iter()
        .collect();
    assert!(
        link_targets_to_place(std::slice::from_ref(&alias), &store_dir, &all, &have_both)
            .is_empty()
    );

    // A link that leaves its directory is not an alias and is left alone.
    let h_climb = "1ccccccccccccccccccccccccccccccc";
    let climb_obj = store_root.join(format!("{h_climb}-ninja-build-x"));
    std::fs::create_dir_all(climb_obj.join("bin")).unwrap();
    std::os::unix::fs::symlink("../lib/libopenfec.so.1.4.2", climb_obj.join("bin/x")).unwrap();
    let climb = enc(h_climb, "ninja-build-x", "bin/x");
    assert!(link_targets_to_place(&[climb], &store_dir, &all, &have).is_empty());
