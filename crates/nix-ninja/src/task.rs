    ...
    fn new_task(&mut self, files: &mut graph::GraphFiles, build: &Build) -> Result<Task> {
        ...
        while let Some((fid, via_phony)) = worklist.pop() {
            ...
            if via_phony && is_gcc_task {
                ...
                if !header_like(p) && !tree_like && compile_artifact_shaped(p) {
                    continue;
                }
            }
...
/// A COMPILED ARTIFACT, as opposed to something a command might READ.
///
/// The phony filter below exists to stop GN's inputdeps phonies dragging
/// generated OBJECTS and SOURCES into one TU's closure, and that is the
/// whole list it may drop. Written as a header allowlist it also dropped
/// DATA a translation unit includes: libid3tag's `src/genre.c` says
/// `# include "genre.dat"` and openal-soft's `core/hrtf.cpp` says
/// `#include "default_hrtf.txt"`, both GENERATED by a custom command that
/// reaches the compile edge only through a `cmake_object_order_depends_*`
/// phony. Neither name is header-shaped, so the filter dropped it from the
/// closure; the scan then found the include, resolved it against the task's
/// declared-virtual set, and hit the guard that skips an upload because the
/// file "is already a Built input of this task" - which this filter had just
/// made false. The compile died on a file that was built and present.
///
/// POLARITY, which is the reason this is a denylist and not a wider
/// allowlist. An extra input costs one upload; a missing one kills the task,
/// and this filter gates what CORRECTNESS needs rather than what a build
/// merely carries. The extension set below is the shapes a compile edge
/// produces beside its object and never includes.
fn compile_artifact_shaped(p: &Path) -> bool {
    matches!(
        p.extension().and_then(|e| e.to_str()),
        Some(
            // objects, archives, shared objects
            "o" | "obj" | "lo" | "a" | "so" | "lib" | "dll" | "dylib"
...
#[cfg(test)]
mod compile_artifact_shaped_tests {
    use super::{compile_artifact_shaped, header_like};
    use std::path::Path;

    /// THE CONSUMER'S TWO FAILURES, and they are the reason the predicate
    /// exists at all. Both files are GENERATED by a custom command that
    /// reaches the compile edge only through a phony, and both are
    /// included by the translation unit: libid3tag's `genre.c` opens
    /// `genre.dat`, openal-soft's `hrtf.cpp` opens `default_hrtf.txt`.
    /// Neither is header-shaped, so the filter that used to run here
    /// dropped them from the closure and the compile died on a file that
    /// had been built.
    #[test]
    fn data_a_translation_unit_includes_is_not_a_compile_artifact() {
        for n in ["genre.dat", "default_hrtf.txt"] {
            assert!(
                !compile_artifact_shaped(Path::new(n)),
                "{n} is reached by an #include and must survive the phony filter"
            );
        }
    }

    /// THE CONTROL, AND IT IS LOAD-BEARING. Without it a predicate that
    /// returned false unconditionally would pass the assertion above, and
    /// the red case would be evidence about nothing. perfetto's `.gen.o`
    /// world is what the filter exists for: thousands of generated objects
    /// and sources riding into one TU's closure through GN's inputdeps
    /// phonies.
    #[test]
    fn generated_objects_and_sources_are_still_dropped() {
        for n in [
            "trace_processor.gen.o",
            "foo.obj",
            "libx.a",
            "liby.so",
            "z.dylib",
            ...
            assert!(
                compile_artifact_shaped(Path::new(n)),
                "{n} is an artifact a compile edge produces, not something it reads"
            );
        }
    }

    /// A header is still carried, because `header_like` is asked first and
    /// this predicate must not be a second, competing answer for it.
    #[test]
