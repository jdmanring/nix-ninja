...
struct BuildImplicitVars<'a> {
    graph: &'a graph::Graph,
    build: &'a graph::Build,
    // ${rspfile} is available inside `command` in real ninja; it was
...
impl<'a> eval::Env for BuildImplicitVars<'a> {
    fn get_var(&self, var: &str) -> Option<EvalString<Cow<str>>> {
        ...
        match var {
            ...
            "rspfile" => self.rspfile_path.clone().and_then(string_to_evalstring),
            _ => None,
        }
    }
}
...
impl Loader {
    ...
    fn add_build(
        ...
        // Resolve the rspfile path FIRST, with rspfile itself out of
        // scope, so ${rspfile} can then be bound while evaluating
        // `command` - matching ninja, where commands reference it.
        let pre_vars = BuildImplicitVars {
            graph: &self.graph,
            build: &build,
            rspfile_path: None,
        };
