...
| 17 | **Code complete, NOT EXERCISED - and the unexercised part is the part just written.** Step one (declared output) and the read-back both existed; what never existed was COLLECTION, so the two halves had never met. Local mode materializes the requested targets only, so no per-object depfile ever reached the build directory and `depfile_read_back` could not fire on any real run. The driver now records each accepted depfile output and local mode materializes them after the targets. Unit-tested gate, six cases. The collection runs in LOCAL mode and every example drives DERIVATION mode, so no build has executed it. It is reachable: `connect_from_env` talks to the daemon socket and only flags `in_drv` off `NIX_BUILD_TOP`, so `nix-ninja <target>` in a configured build directory against a `builder-rpc-v0` daemon exercises it. That is the test to write, and until someone writes it this row is not evidence |
